...
function getCategories() {
    ...
    $.ajax({
        url: "/textAnalyse/getCategories",
        type: "post",
        dataType: "json",
        success: function (jsonArray) {
            $.each(jsonArray, function (index, json) {
                $("#select").append("<option value='" + json['category'] + "'>" + json['category'] + "</option>");
            });
            $("#select").change(function () {
                var selectedValue = $("#select").val();
                getWordCloudImageData(selectedValue);
            });
            getWordCloudImageData($("#select").val());
        }
    });
}

function getWordCloudImageData(category) {
    $.ajax({
        url: "/textAnalyse/getKeywordStatistics",
        ...
        success: function (json) {
            ...
            $.each(results, function (key) {
                data.push({
                    name: key, value: parseInt(results[key])
                });
            });
